...
export const AudioProvider = ({ children }) => {
    ...
    const handleSaveNoteProvider = async ({ title, content, color, image, date, photoUri }) => {
        ...
        try {
            const response = await axios.post('http://192.168.8.141:3000/api/notes', noteData);
    ...
    const handleUpdateNoteProvider = async ({ id, title, content, color, image, date, photoUri }) => {
        ...
        try {
            ...
            const url = `http://192.168.8.141:3000/api/notes/${id}`;
    ...
    const getNotes = async () => {
        ...
        try {
            const response = await axios.get('http://192.168.8.141:3000/api/notes');
